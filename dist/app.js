...
if(this.gameSounds.loaded||this.gameSounds.loading){return}var sounds=this.colors.concat("wrong");var srcs={green:"assets/sounds/NFF-good-tip-high.mp3",red:"assets/sounds/NFF-good-tip-low.mp3",yellow:"assets/sounds/NFF-good-tip-high-2.mp3",blue:"assets/sounds/NFF-good-tip-low-2.mp3",wrong:"assets/sounds/NFF-wrong.mp3"};this.gameSounds.green=new Audio;this.gameSounds.green.play();this.gameSounds.red=new Audio;this.gameSounds.red.play();this.gameSounds.yellow=new Audio;this.gameSounds.yellow.play();this.gameSounds.blue=new Audio;this.gameSounds.blue.play();this.gameSounds.wrong=new Audio;this.gameSounds.wrong.play();this.gameSounds.loading=true;var promises=[];var game=this;var _loop=function _loop(i,len){promises.push(new Promise(function(resolve,reject){game.gameSounds[sounds[i]].oncanplaythrough=function(){resolve(true)};game.gameSounds[sounds[i]].src=srcs[sounds[i]]}))};for(var i=0,len=sounds.length;i<len;i++){_loop(i,len)}return new Promise(function(resolve,reject){Promise.all(promises).then(function(){game.gameSounds.loading=false;game.gameSounds.loaded=true;resolve(true)})})},setMainBtn:function setMainBtn(msg){document.querySelector("#info-main-btn").innerHTML=msg},setLevelField:function setLevelField(){var level=this.playerLevel||0;document.querySelector("#info-level").innerHTML=level},setStrictField:function setStrictField(){var res="Strict Mode "+(this.strictMode?"On":"Off");document.querySelector("#info-strict").innerHTML=res},// Input Handling
